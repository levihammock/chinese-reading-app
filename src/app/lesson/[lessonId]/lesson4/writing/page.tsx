...
export default function WritingPage({ params }: PageProps) {
    ...
    const loadParams = async () => {
      try {
        ...
        if (isMounted) {
          ...
          const quiz = (data.lessonData.writingQuiz as WritingQuiz) || null;
          setWritingQuiz(quiz);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const requestData = {
        questions: writingQuiz,
        answers: answers,
        skillLevel: lessonData?.skillLevel || 'HSK1',
        topic: lessonData?.topic || 'General'
      };
      
      console.log('Sending writing quiz data:', requestData);
      ...
      if (response.ok) {
        ...
        console.log('Writing quiz evaluation result:', result);
        setEvaluations(result.evaluations || []);
        setSummary(result.summary || null);
      } else {
        const errorText = await response.text();
        console.error('Writing quiz evaluation error:', errorText);
        alert('Error evaluating writing quiz: ' + errorText);
      }
    } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-[#00AFB9]">Loading writing quiz...</p>
        </div>
      </div>
    );
  }
  ...
  if (!writingQuiz) {
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-4xl font-bold text-[#0081A7] mb-4">KanKan</h1>
          <p className="text-red-500 mb-4">Writing quiz not available</p>
          <button
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="ml-64 p-8">
        <div className="max-w-4xl mx-auto">
          <h1 className="text-4xl font-bold text-[#0081A7] mb-8 text-center">Writing Quiz</h1>
          
          {evaluations.length === 0 ? (
            <div className="space-y-6">
              {writingQuiz.map((question, index) => (
                <div key={index} className="bg-white rounded-xl shadow-lg p-6 border-2 border-[#FED9B7]">
                  <div className="mb-4">
                    <h3 className="text-xl font-semibold text-[#0081A7] mb-2">
                      Question {index + 1}: {question.english}
                    </h3>
                  </div>
                  ...
                  <div className="mb-4">
                    ...
                    <textarea
                      ...
                      rows={3}
                      placeholder="Enter your Chinese translation..."
                    />
                  </div>
                </div>
              ))}
            ...
            <div className="space-y-6">
              {evaluations.map((evaluation, index) => {
                const question = writingQuiz[evaluation.questionIndex];
                return (
                  <div key={index} className="bg-white rounded-xl shadow-lg p-6 border-2 border-[#FED9B7]">
                    <div className="mb-4">
                      <h3 className="text-xl font-semibold text-[#0081A7] mb-2">
                        Question {evaluation.questionIndex + 1}: {question?.english || 'Unknown'}
                      </h3>
                    </div>
